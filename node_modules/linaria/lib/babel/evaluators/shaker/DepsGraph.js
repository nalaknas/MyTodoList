...
class DepsGraph {
  ...
  getDependencies(nodes) {
    this.processQueue();
    return nodes.reduce((acc, node) => acc.concat(Array.from(this.dependencies.get(node) || [])), []);
  }

  getLeafs(only) {
    this.processQueue();
    return only ? only.map(name => this.exports.get(name)) : Array.from(this.exports.values());
  }
